#!/usr/bin/env bash
# ══════════════════════════════════════════════════════════════════════════════
# OpenRFStack recon node entrypoint (passive collection — no on-device ID)
# Enabled services: sdr_controller, sdr_acquisition, sdr_gps, sdr_recon
# ══════════════════════════════════════════════════════════════════════════════
set -euo pipefail
...
log()  { echo -e "${BLU}[recon-node]${RST} $*"; }
ok()   { echo -e "${GRN}[recon-node]${RST} $*"; }
warn() { echo -e "${YLW}[recon-node]${RST} $*"; }
...
mkdir -p "$SDR_DATA"/{pgdata,gps,acq-cache} "$RECON_OUT"
...
for vf in glob.glob('/sys/bus/usb/devices/*/idVendor'):
    try:
        ...
        os.close(fd)
        import sys; print(f'[recon-node] Reset RTL-SDR at {path}', file=sys.stderr)
...
if [[ "$ENABLE_SDR_CONTROLLER" == "true" ]]; then
    [[ -x /usr/bin/sdr_controller ]] \
        && start_service sdr-controller /usr/bin/sdr_controller "$SDR_ETC/devices.xml" \
        || warn "Binary /usr/bin/sdr_controller not found — skipping sdr-controller"
fi
if [[ "$ENABLE_SDR_ACQUISITION" == "true" ]]; then
    [[ -x /usr/bin/sdr_acquisition ]] \
        && start_service sdr-acquisition /usr/bin/sdr_acquisition "$SDR_ETC/scanner.xml" \
        || warn "Binary /usr/bin/sdr_acquisition not found — skipping sdr-acquisition"
fi
if [[ "$ENABLE_SDR_GPS" == "true" ]]; then
    [[ -x /usr/bin/sdr_gps ]] \
        && start_service sdr-gps /usr/bin/sdr_gps /etc/sdr-gps/gps.xml \
        || warn "Binary /usr/bin/sdr_gps not found — skipping sdr-gps"
fi
if [[ "$ENABLE_SDR_RECON" == "true" ]]; then
    # Defaults (--broker, --user, --password, queue/topic names) already
    # match this stack's Artemis credentials and AcquisitionApp's config —
    # see deploy/recon-node/configs/scanner.xml and k8s/01-artemis.yaml.
    start_service sdr-recon /usr/bin/python3 /usr/local/bin/sdr_recon.py \
        --out "$RECON_OUT" \
