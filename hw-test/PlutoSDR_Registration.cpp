...
static std::vector<SoapySDR::Kwargs> find_PlutoSDR(const SoapySDR::Kwargs &args) {
	...
	// Allow PLUTO_IP env var to override hostname discovery
	SoapySDR::Kwargs mutable_args = args;
	const char *pluto_ip_env = std::getenv("PLUTO_IP");
	if (pluto_ip_env && *pluto_ip_env && mutable_args.count("hostname") == 0) {
		mutable_args["hostname"] = pluto_ip_env;
	}
	const SoapySDR::Kwargs &effective_args = mutable_args;
	...
	for (std::vector<std::string>::iterator it = backends.begin(); it != backends.end(); it++) {
		...
		if (ret < 0) {
			...
			info = nullptr;
			scan_ctx = nullptr;
			// For the ip backend, fall through to hostname fallback below
			if (*it != "ip") continue;
			ret = 0; // treat as "no devices found" so hostname fallback runs
		}

		options["device"] = "PlutoSDR";
		if (ret == 0) {
			if (info) iio_context_info_list_free(info);
			if (scan_ctx) iio_scan_context_destroy(scan_ctx);

			//no devices discovered, the user must specify a hostname
			if (effective_args.count("hostname") == 0) continue;

			//try to connect at the specified hostname
			ctx = iio_create_network_context(effective_args.at("hostname").c_str());
			if (ctx == nullptr) continue; //failed to connect
			options["hostname"] = effective_args.at("hostname");
